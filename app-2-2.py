...
with t5:
    ...
    # Build styled HTML table — visible on dark background
    table_rows = ""
    for row in rows:
        yoy_val = row["YoY"].replace("+","").replace("%","")
        try:
            yoy_f = float(yoy_val)
            yoy_col = "#E74C3C" if yoy_f > 0.5 else "#2ECC71" if yoy_f < -0.5 else "#F1C40F"
        except:
            yoy_col = "white"
        table_rows += (
            f"<tr style='border-bottom:1px solid rgba(255,255,255,0.06);'>"
            f"<td style='padding:10px 12px;color:white;font-weight:500;'>{row['Category']}</td>"
            f"<td style='padding:10px 12px;color:rgba(255,255,255,0.6);text-align:center;'>{row['Shelf Life']}</td>"
            f"<td style='padding:10px 12px;color:rgba(255,255,255,0.7);text-align:center;'>{row['Rate 2024']}</td>"
            f"<td style='padding:10px 12px;color:white;font-weight:600;text-align:center;'>{row['Rate 2025']}</td>"
            f"<td style='padding:10px 12px;color:{yoy_col};font-weight:600;text-align:center;'>{row['YoY']}</td>"
            f"<td style='padding:10px 12px;color:rgba(255,255,255,0.6);text-align:center;'>{row['Trend']}</td>"
            f"</tr>"
        )
    st.markdown(
        f"<div style='overflow-x:auto;'>"
        f"<table style='width:100%;border-collapse:collapse;"
        f"background:#0D1B2A;border-radius:8px;overflow:hidden;'>"
        f"<thead>"
        f"<tr style='background:#1A2F45;'>"
        f"<th style='padding:12px;color:#E67E22;text-align:left;font-size:12px;letter-spacing:1px;'>CATEGORY</th>"
        f"<th style='padding:12px;color:#E67E22;text-align:center;font-size:12px;letter-spacing:1px;'>SHELF LIFE</th>"
        f"<th style='padding:12px;color:#E67E22;text-align:center;font-size:12px;letter-spacing:1px;'>RATE 2024</th>"
        f"<th style='padding:12px;color:#E67E22;text-align:center;font-size:12px;letter-spacing:1px;'>RATE 2025</th>"
        f"<th style='padding:12px;color:#E67E22;text-align:center;font-size:12px;letter-spacing:1px;'>YOY</th>"
        f"<th style='padding:12px;color:#E67E22;text-align:center;font-size:12px;letter-spacing:1px;'>TREND</th>"
        f"</tr>"
        f"</thead>"
